...
function Filter({ filters, isActive, handleClick, handleApplyFilters, handleFilterChange }) {
    ...
    return (
        <>
            <button
                className="max-w-min flex flex-row items-center gap-1 py-2 px-4  rounded-md shadow-md shadow-gray-400 border-2 border-gray-100 bg-gray-50 lg:hover:border-fg-green-400"
                onClick={handleClick}
            >
                <IoFilter className="text-fg-green-400"/>
                <p className="text-[1rem] font-semibold">Filters</p>
            </button>
            <div className={` ${isActive ? 'visible' : 'invisible'} position absolute translate-y-11 rounded-md shadow-lg shadow-gray-400 flex flex-col gap-2 border-gray-200/50 border-2 bg-gray-100 px-4 py-2 w-56 mt-1 z-50`}>
